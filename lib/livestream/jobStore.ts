...
function assembleJob(
  ...
  return {
    ...
    negativePromptOverride: jobRow.negativePromptOverride,
    ...
    // null (job tạo trước khi có cột) = chưa tắt khối nào = bật hết, giữ đúng hành vi cũ.
    disabledPromptBlocks: jobRow.disabledPromptBlocks ?? [],
...
function jobToRow(job: LivestreamJob): Omit<typeof livestreamJobs.$inferInsert, 'id'> {
  return {
    ...
    negativePromptOverride: job.negativePromptOverride ?? null,
    ...
    disabledPromptBlocks: job.disabledPromptBlocks ?? [],
...
async function persistJob(tx: Tx, job: LivestreamJob, existingDbId: number | null): Promise<number> {
  ...
  if (job.products.length > 0) {
    const productValuesList = job.products.map((product) => ({
      ...
      sourceRaw: product.sourceRaw ?? null,
    ...
    await tx
      ...
      .onDuplicateKeyUpdate({
        set: {
          ...
          sourceRaw: sql`values(${livestreamProducts.sourceRaw})`,
