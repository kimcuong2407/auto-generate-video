 ...
 *
 * Cách kiểm: quét mã nguồn, mỗi lời gọi chatCompletion/generateScriptText trong lib/livestream và
 * lib/data phải lấy system prompt từ registry (`loadPromptSet(...).get(...)` hoặc biến
 * `systemPrompt`), KHÔNG được truyền thẳng hằng *_SYSTEM_PROMPT.
 *
 * Vùng quét PHẢI phủ cả luồng Video Review (lib/data, app/api/projects), không chỉ livestream:
 * bốn bước review từng nằm ngoài vùng quét nên bị hard-code prompt suốt một thời gian dài mà
 * check này vẫn báo xanh.
 *
...
const LIB_DIRS = [path.join(process.cwd(), 'lib/livestream'), path.join(process.cwd(), 'lib/data')];
const files = LIB_DIRS.flatMap((d) =>
...
for (const f of files) {
  ...
  for (const m of src.matchAll(CALL_WITH_CONST)) {
    offenders.push(`${path.basename(f)}: truyền thẳng ${m[1]} cho AI thay vì lấy từ registry`);
  }
}
assert.deepEqual(
  offenders,
