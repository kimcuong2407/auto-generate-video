...
/**
 * Upload/thêm ảnh sản phẩm vào KHO ẢNH CHUNG cấp job (job.spokespersonImagePaths) — dùng làm
 * refPaths (character reference) khi gen video, áp cho MỌI segment của MỌI sản phẩm. Người dùng
 * chọn 1 ảnh trong kho này làm ref chính qua route images/select-ref (kind='product'). Mỗi lần
 * POST sẽ APPEND thêm (không ghi đè).
 */
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Đặt tên duy nhất theo thời điểm + index để không đè ảnh đã có (append).
  ...
  for (const file of files) {
    ...
    const fileName = `spokesperson-${stamp}-${k}${ext}`;
  ...
  // Đẩy từng ảnh mới lên R2 (best-effort) để không mất khi deploy; lưu URL theo relPath.
  ...
  const { job: updatedJob } = await updateJob(id, (j) => {
    if (!Array.isArray(j.spokespersonImagePaths)) j.spokespersonImagePaths = [];
    j.spokespersonImagePaths.push(...newRelPaths);
...
/**
 * Xoá ảnh sản phẩm khỏi kho chung. Truyền query ?path=inputs/xxx.jpg để xoá đúng 1 ảnh; không
 * truyền path thì xoá toàn bộ. Nếu ảnh bị xoá đang được chọn (selectedRefImagePath) thì clear.
 */
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const current = job.spokespersonImagePaths ?? [];

  // Danh sách relPath sẽ xoá — để dọn cả file local, object R2 và key trong imageR2Urls.
